...
function LiquidationBubblePage() {
  ...
  useEffect(() => {
    ...
    chart.setOption({ animation: false, backgroundColor: 'transparent', grid: { left: 58, right: 28, top: 44, bottom: 68 }, legend: { top: 8, textStyle: { color: '#9eacc1', fontSize: 11 }, data: ['OKX 5m K线', '多头爆仓', '空头爆仓'] }, tooltip: { trigger: 'axis', axisPointer: { type: 'cross' }, backgroundColor: '#101925', borderColor: '#3a4b65', textStyle: { color: '#e9eef7' }, formatter: (raw: unknown) => { const items = raw as Array<{ seriesName: string, value: unknown[] }>; return items.filter(item => item.seriesName !== 'OKX 5m K线').map(item => `<b>${item.seriesName}</b><br/>时间：${new Date(String(item.value[0])).toLocaleString()}<br/>价格：${Number(item.value[5] ?? item.value[1]).toLocaleString()}<br/>名义价值：${formatMoney(Number(item.value[2]))}<br/>交易所：${item.value[3]}<br/>数量：${Number(item.value[4]).toLocaleString()}`).join('<hr/>') } }, xAxis: { type: 'time', axisLine: { lineStyle: { color: '#415069' } }, axisLabel: { color: '#8492a7' }, splitLine: { show: false } }, yAxis: { scale: true, axisLine: { lineStyle: { color: '#415069' } }, axisLabel: { color: '#8492a7' }, splitLine: { lineStyle: { color: '#243044' } } }, dataZoom: [{ type: 'inside', xAxisIndex: 0, zoomOnMouseWheel: true, moveOnMouseMove: true, moveOnMouseWheel: false, preventDefaultMouseMove: true, ...zoomState }, { type: 'slider', xAxisIndex: 0, height: 18, bottom: 18, borderColor: '#314058', fillerColor: '#725cbb55', textStyle: { color: '#8492a7' }, ...zoomState }], series: [{ name: 'OKX 5m K线', type: 'candlestick', data: candles.map(item => [item.openTime, item.open, item.close, item.low, item.high]), itemStyle: { color: '#57c995', color0: '#ec6e83', borderColor: '#57c995', borderColor0: '#ec6e83' } }, bubbles('long', '#f06e84', '多头爆仓'), bubbles('short', '#55d7a1', '空头爆仓')] }, true); if (candles.length === 0) chart.clear()
  }, [data])
