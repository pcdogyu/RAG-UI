...
import { activeRiskSignalCount, directionalLevels, ethRiskSymbol, riskRanges, type RiskRange } from './riskRadar'

echarts.use([CandlestickChart, LineChart, ScatterChart, DataZoomComponent, GridComponent, LegendComponent, TooltipComponent, CanvasRenderer])
...
type ETHRiskSnapshot = RiskSnapshot & { cvd_total_usd: number }
type ETHRiskForecastLevel = { side: 'long' | 'short', price: number, intensity: number, distance_pct: number }
type ETHRiskForecast = { levels: ETHRiskForecastLevel[], status: 'ok' | 'stale' | 'unavailable', updated_at?: string, source: string }
type ETHRiskRadar = { snapshot: ETHRiskSnapshot, observed_levels: RiskZone[], forecast: ETHRiskForecast, signals: RiskSignal[], events: RiskEvent[], status: string }
type ETHRiskHistory = { symbol: string, range: RiskRange, snapshots: ETHRiskSnapshot[], status: string }
const compactRiskNumber = (value: number) => new Intl.NumberFormat('zh-CN', { notation: 'compact', maximumFractionDigits: 2 }).format(value)

function ETHRiskTrendChart({ title, unit, color, data, field, theme }: { title: string, unit: string, color: string, data: ETHRiskSnapshot[], field: 'oi_usd' | 'cvd_total_usd', theme: Theme }) {
  const element = useRef<HTMLDivElement>(null); const instance = useRef<echarts.ECharts | null>(null)
  useEffect(() => { if (!element.current) return; const chart = echarts.init(element.current); instance.current = chart; const observer = new ResizeObserver(() => chart.resize()); observer.observe(element.current); return () => { observer.disconnect(); chart.dispose(); instance.current = null } }, [])
  useEffect(() => { const chart = instance.current; if (!chart) return; const light = theme === 'light'; const muted = light ? '#60738b' : '#94a3b8'; const grid = light ? '#d7e0ec' : '#26364a'; chart.setOption({ animation: false, grid: { left: 62, right: 18, top: 30, bottom: 34 }, tooltip: { trigger: 'axis', backgroundColor: light ? '#fff' : '#101925', borderColor: light ? '#b6c4d6' : '#3a4b65', textStyle: { color: light ? '#18243a' : '#e9eef7' }, formatter: (items: unknown) => { const point = (items as Array<{ value: [string, number] }>)[0]; return point ? `${new Date(point.value[0]).toLocaleString()}<br/><b>${title}</b>：${compactRiskNumber(point.value[1])} ${unit}` : '' } }, xAxis: { type: 'time', axisLabel: { color: muted, hideOverlap: true }, axisLine: { lineStyle: { color: muted } }, splitLine: { show: false } }, yAxis: { type: 'value', scale: true, axisLabel: { color: muted, formatter: (value: number) => compactRiskNumber(value) }, splitLine: { lineStyle: { color: grid } } }, series: [{ name: title, type: 'line', showSymbol: data.length <= 1, symbolSize: 9, smooth: true, lineStyle: { color, width: 2 }, areaStyle: { color: `${color}22` }, data: data.map(item => [item.observed_at, item[field]]) }] }, true); if (!data.length) chart.clear() }, [color, data, field, theme, title, unit])
  return <div className="risk-trend-chart" ref={element} aria-label={`${title}折线图`} />
}

function ETHRiskLevelColumn({ title, tone, levels, observed }: { title: string, tone: 'long' | 'short', levels: Array<RiskZone | ETHRiskForecastLevel>, observed: boolean }) {
  return <div className={`risk-level-column ${tone}`}><div><b>{title}</b><span>{observed ? '实际强平观测 · 24h 衰减' : '外部模型预测压力区 · 30d'}</span></div>{levels.length ? <ol>{levels.map(level => <li key={`${level.price}-${level.side}`}><b>${level.price.toLocaleString(undefined, { maximumFractionDigits: 4 })}</b><span>距现价 {level.distance_pct.toFixed(2)}% · {observed ? riskMoney((level as RiskZone).notional_usd) : `强度 ${compactRiskNumber((level as ETHRiskForecastLevel).intensity)}`}</span></li>)}</ol> : <p>{observed ? '暂无足够的同向实际强平事件。' : '预测源暂无该方向有效压力区。'}</p>}</div>
}

function ETHRiskPage({ theme, alertCount, onAlertCount }: { theme: Theme, alertCount: number, onAlertCount: (count: number) => void }) {
  const [radar, setRadar] = useState<ETHRiskRadar | null>(null); const [history, setHistory] = useState<ETHRiskHistory | null>(null); const [range, setRange] = useState<RiskRange>('24h'); const [error, setError] = useState(''); const [loading, setLoading] = useState(true)
  useEffect(() => { let active = true; const load = async () => { setLoading(true); try { const [snapshotResponse, historyResponse] = await Promise.all([fetch(`/api/v1/risk-radar/snapshot?symbol=${ethRiskSymbol}`, { cache: 'no-store' }), fetch(`/api/v1/risk-radar/history?range=${range}`, { cache: 'no-store' })]); const snapshot = await snapshotResponse.json() as ETHRiskRadar & { error?: string }; const historyResult = await historyResponse.json() as ETHRiskHistory & { error?: string }; if (!snapshotResponse.ok) throw new Error(snapshot.error || '风险雷达尚未准备完成'); if (!historyResponse.ok) throw new Error(historyResult.error || '无法读取 OI / CVD 历史'); if (active) { setRadar(snapshot); setHistory(historyResult); onAlertCount(activeRiskSignalCount(snapshot.signals)); setError('') } } catch (reason) { if (active) { setRadar(null); setHistory(null); setError(reason instanceof Error ? reason.message : '风险雷达加载失败') } } finally { if (active) setLoading(false) } }; void load(); const timer = window.setInterval(() => void load(), 30_000); return () => { active = false; window.clearInterval(timer) } }, [onAlertCount, range])
  const snapshot = radar?.snapshot; const activeSignals = activeRiskSignalCount(radar?.signals); const latest = radar?.events[0]; const observed = radar?.observed_levels ?? []; const forecast = radar?.forecast; const nearest = observed.slice().sort((left, right) => left.distance_pct - right.distance_pct)[0]; const longs = directionalLevels(observed, 'long'); const shorts = directionalLevels(observed, 'short'); const forecastLongs = directionalLevels(forecast?.levels ?? [], 'long'); const forecastShorts = directionalLevels(forecast?.levels ?? [], 'short')
  return <div className="risk-page"><div className="risk-toolbar"><b>ETH-USDT</b><span>Binance / OKX 公开永续数据 · 30 秒评估 · 仅研究提示</span></div>{error ? <Card className="risk-error"><b>ETH 风险雷达暂不可用</b><p>{error}</p><small>需要 PostgreSQL 与公开市场数据完成首个 5 分钟快照；页面不会显示示例数据。</small></Card> : <><div className="risk-banner"><div><div className="eyebrow">ETH-USDT · OBSERVED LIQUIDATION RISK</div><h1>{latest ? `${latest.level} 风险事件` : activeSignals >= 2 ? '风险信号共振' : '杠杆风险观察'}</h1><p>实际强平密度、OI、Funding、CVD 与基差 · 仅研究提示，不构成交易建议</p></div><div className="risk-level"><span>当前等级</span><b>{latest?.level ?? 'OBSERVE'}</b><em>{snapshot ? `数据快照 ${new Date(snapshot.observed_at).toLocaleTimeString()}` : '等待首个快照'}</em></div></div><div className="metrics">{[['价格', snapshot ? `$${snapshot.mark_price.toLocaleString(undefined, { maximumFractionDigits: 6 })}` : '—', snapshot ? `现货 $${snapshot.spot_price.toLocaleString(undefined, { maximumFractionDigits: 6 })}` : '', 'green'], ['最近实际热区', nearest ? nearest.price.toLocaleString(undefined, { maximumFractionDigits: 6 }) : '—', nearest ? `${nearest.side === 'long' ? '多单' : '空单'} · 距现价 ${nearest.distance_pct.toFixed(2)}%` : '尚无足够强平事件', 'amber'], ['聚合未平仓合约', snapshot ? riskMoney(snapshot.oi_usd) : '—', snapshot ? `${snapshot.venue_count} 个交易所数据源` : '', 'red'], ['Funding / 5 分钟 CVD', snapshot ? `${(snapshot.funding_rate * 100).toFixed(4)}%` : '—', snapshot ? `基差 ${snapshot.basis_pct.toFixed(3)}% · ${riskMoney(snapshot.cvd_delta_usd)}` : '', 'violet']].map(([label, value, detail, tone]) => <Card key={label}><label>{label}</label><h2>{value}</h2><Pill tone={tone}>{detail || '采集中'}</Pill></Card>)}</div><Card className="risk-trends"><div className="card-heading"><div><div className="eyebrow">ETH MARKET HISTORY</div><h2>OI / CVD 趋势</h2><p>5 分钟快照；CVD 为跨服务重启持续累计的主动成交净额。</p></div><div className="risk-range" role="group" aria-label="OI 和 CVD 时间范围">{riskRanges.map(item => <button key={item} className={range === item ? 'active' : ''} onClick={() => setRange(item)}>{item}</button>)}</div></div><div className="risk-trend-grid"><div><b>聚合 OI</b><ETHRiskTrendChart title="聚合 OI" unit="USDT" color="#72a7ff" data={history?.snapshots ?? []} field="oi_usd" theme={theme} /></div><div><b>CVD 累计主动成交量</b><ETHRiskTrendChart title="CVD 累计主动成交量" unit="USDT" color="#a788ff" data={history?.snapshots ?? []} field="cvd_total_usd" theme={theme} /></div></div>{history?.status === 'collecting' || (!loading && !(history?.snapshots.length)) ? <p className="risk-chart-empty">正在积累 ETH 的 5 分钟历史快照，曲线将在采集后自动出现。</p> : null}</Card><Card className="risk-levels"><div className="card-heading"><div><div className="eyebrow">LONG / SHORT LIQUIDATION LEVELS</div><h2>多空清算水平</h2><p>实际强平热区与外部模型预测压力区分开显示；预测不代表交易所精确清算价。</p></div><Pill tone={forecast?.status === 'ok' ? 'violet' : forecast?.status === 'stale' ? 'amber' : 'muted'}>{forecast?.status === 'ok' ? '预测源正常' : forecast?.status === 'stale' ? '预测数据已缓存' : '预测源不可用'}</Pill></div><div className="risk-level-grid"><ETHRiskLevelColumn title="多单实际热区" tone="long" levels={longs} observed /><ETHRiskLevelColumn title="空单实际热区" tone="short" levels={shorts} observed /><ETHRiskLevelColumn title="多单预测压力区" tone="long" levels={forecastLongs} observed={false} /><ETHRiskLevelColumn title="空单预测压力区" tone="short" levels={forecastShorts} observed={false} /></div><p className="risk-level-source">{forecast?.source ?? '外部模型预测压力区 / 30 天窗口'}{forecast?.updated_at ? ` · 数据时间 ${new Date(forecast.updated_at).toLocaleString()}` : ''}</p></Card><div className="two-columns"><Card><div className="card-heading"><div><div className="eyebrow">EXPLAINABLE SIGNALS</div><h2>共振触发依据</h2></div><Pill tone={alertCount >= 2 ? 'red' : 'muted'}>{alertCount} / 5 维度</Pill></div>{radar?.signals.map(signal => <div className="signal" key={signal.id}><span className={signal.active ? 'signal-on' : ''} /><div><b>{signal.title}</b><p>{signal.detail}</p></div><Pill tone={signal.active ? riskTone(signal.severity === 'high' ? 'HIGH' : 'MEDIUM') : 'muted'}>{signal.active ? '触发' : '观察'}</Pill></div>) ?? <p>{loading ? '正在读取最新风险快照…' : '暂无风险信号。'}</p>}</Card><Card><div className="eyebrow">LATEST EVENT</div><h2>{latest ? `风险提示 #${latest.id}` : '暂无主动风险事件'}</h2><p className="detail-meta">{latest ? `${latest.symbol} · ${new Date(latest.created_at).toLocaleString()}` : '至少两项维度显著变化才生成事件'}</p><div className="detail-section"><label>触发说明</label><p>{latest ? latest.signals.filter(item => item.active).map(item => item.title).join('、') : '当前仅保留观察，不发送 Telegram。'}</p></div><div className="detail-section"><label>Telegram 投递</label><Pill tone={latest?.telegram_status === 'sent' ? 'green' : latest?.telegram_status === 'failed' ? 'red' : 'muted'}>{latest?.telegram_status ?? '未触发'}</Pill></div></Card></div></>}</div>
}
...
function App() {
  const [page, setPage] = useState<Page>('ask'); const [collapsed, setCollapsed] = useState(false); const [theme, setTheme] = useState<Theme>(storedTheme); const [riskAlertCount, setRiskAlertCount] = useState(0)
  useEffect(() => { document.documentElement.dataset.theme = theme; localStorage.setItem(themeStorageKey, theme) }, [theme])
  useEffect(() => { let active = true; const refresh = async () => { try { const response = await fetch(`/api/v1/risk-radar/snapshot?symbol=${ethRiskSymbol}`, { cache: 'no-store' }); const result = await response.json() as Pick<ETHRiskRadar, 'signals'> & { error?: string }; if (response.ok && active) setRiskAlertCount(activeRiskSignalCount(result.signals)) } catch {} }; void refresh(); const timer = window.setInterval(() => void refresh(), 30_000); return () => { active = false; window.clearInterval(timer) } }, [])
  const render = () => ({ask:<AskPage />, memory:<MemoryPage />, studio:<StudioPage />, risk:<ETHRiskPage theme={theme} alertCount={riskAlertCount} onAlertCount={setRiskAlertCount} />, liquidationMap:<LiquidationMapPage />, liquidation:<LiquidationBubblePage theme={theme} />, watchlist:<WatchlistPage />, cockpit:<CockpitPage />, sources:<SettingsPage page="sources" />, rules:<SettingsPage page="rules" />, audit:<SettingsPage page="audit" />}[page])
  const [title, subtitle] = pageMeta[page]
  return <div className={`app-shell ${collapsed ? 'collapsed' : ''}`} data-theme={theme}><aside><div className="brand"><span>R</span><div><b>RESEARCH OS</b><small>机构研究工作台</small></div><button onClick={() => setCollapsed(!collapsed)}>☰</button></div><nav>{navGroups.map(group => <div className="nav-group" key={group.label}><label>{group.label}</label>{group.items.map(([id, icon, text]) => <button key={id} onClick={() => setPage(id)} className={page === id ? 'active' : ''}><i>{icon}</i><span>{text}</span>{id === 'risk' && riskAlertCount > 0 && <em>{riskAlertCount}</em>}</button>)}</div>)}</nav><div className="user"><div className="avatar">LR</div><div><b>林然</b><small>研究负责人</small></div><span>⌄</span></div></aside><main><header><div><div className="breadcrumb">RESEARCH OS / {title}</div><h1>{title}</h1><p>{subtitle}</p></div><div className="header-actions"><div className="theme-switcher" role="group" aria-label="界面主题"><button type="button" aria-pressed={theme === 'light'} className={theme === 'light' ? 'active' : ''} onClick={() => setTheme('light')}>☀ 浅色</button><button type="button" aria-pressed={theme === 'dark'} className={theme === 'dark' ? 'active' : ''} onClick={() => setTheme('dark')}>☾ 深色</button></div><button className="notice" aria-label={`当前 ${riskAlertCount} 个风险因子触发`}>◉{riskAlertCount > 0 && <span>{riskAlertCount}</span>}</button><span className="status"><i /> HYGR 智能问答已连接</span></div></header><div className="page-content">{render()}</div><BuildFooter /></main></div>
}
