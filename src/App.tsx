...
function LiquidationBubblePage({ theme }: { theme: Theme }) {
  ...
  useEffect(() => {
    ...
    const bubbles = (side: 'long' | 'short', color: string, name: string) => ({
      ...
      symbolOffset: [0, side === 'long' ? 12 : -12],
      symbolSize: (value: unknown[]) => Math.min(46, Math.max(8, Math.log10(Number(value[2]) + 1) * 5)), itemStyle: { color, opacity: .72 }, z: 6
    })
    const chartTheme = theme === 'light'
      ? { muted: '#52657c', grid: '#d7e0ec', tooltip: '#ffffff', tooltipBorder: '#b6c4d6', text: '#18243a', sliderBorder: '#b8c6d8', sliderFill: '#725cbb40' }
      : { muted: '#9eacc1', grid: '#243044', tooltip: '#101925', tooltipBorder: '#3a4b65', text: '#e9eef7', sliderBorder: '#314058', sliderFill: '#725cbb55' }
    chart.setOption({ animation: false, backgroundColor: 'transparent', grid: { left: 58, right: 28, top: 44, bottom: 68 }, legend: { top: 8, textStyle: { color: chartTheme.muted, fontSize: 11 }, data: ['OKX 5m K线', '多头爆仓', '空头爆仓'] }, tooltip: { trigger: 'axis', axisPointer: { type: 'cross' }, backgroundColor: chartTheme.tooltip, borderColor: chartTheme.tooltipBorder, textStyle: { color: chartTheme.text }, formatter: (raw: unknown) => { const items = raw as Array<{ seriesName: string, value: unknown[] }>; return items.filter(item => item.seriesName !== 'OKX 5m K线').map(item => `<b>${item.seriesName}</b><br/>时间：${new Date(String(item.value[0])).toLocaleString()}<br/>价格：${Number(item.value[5] ?? item.value[1]).toLocaleString()}<br/>名义价值：${formatMoney(Number(item.value[2]))}<br/>交易所：${item.value[3]}<br/>数量：${Number(item.value[4]).toLocaleString()}`).join('<hr/>') } }, xAxis: { type: 'time', axisLine: { lineStyle: { color: chartTheme.muted } }, axisLabel: { color: chartTheme.muted }, splitLine: { show: false } }, yAxis: { scale: true, axisLine: { lineStyle: { color: chartTheme.muted } }, axisLabel: { color: chartTheme.muted }, splitLine: { lineStyle: { color: chartTheme.grid } } }, dataZoom: [{ type: 'inside', xAxisIndex: 0, zoomOnMouseWheel: true, moveOnMouseMove: true, moveOnMouseWheel: false, preventDefaultMouseMove: true, ...zoomState }, { type: 'slider', xAxisIndex: 0, height: 18, bottom: 18, borderColor: chartTheme.sliderBorder, fillerColor: chartTheme.sliderFill, textStyle: { color: chartTheme.muted }, ...zoomState }], series: [{ name: 'OKX 5m K线', type: 'candlestick', data: candles.map(item => [item.openTime, item.open, item.close, item.low, item.high]), itemStyle: { color: '#57c995', color0: '#ec6e83', borderColor: '#57c995', borderColor0: '#ec6e83' } }, bubbles('long', '#f06e84', '多头爆仓'), bubbles('short', '#55d7a1', '空头爆仓')] }, true); if (candles.length === 0) chart.clear()
  }, [data, theme])
...
function App() {
  ...
  const render = () => ({ask:<AskPage />, memory:<MemoryPage />, studio:<StudioPage />, risk:<RiskPage />, liquidation:<LiquidationBubblePage theme={theme} />, watchlist:<WatchlistPage />, cockpit:<CockpitPage />, sources:<SettingsPage page="sources" />, rules:<SettingsPage page="rules" />, audit:<SettingsPage page="audit" />}[page])
