...
function LiquidationBubblePage({ theme }: { theme: Theme }) {
  ...
  const [symbols, setSymbols] = useState<LiquidationSymbol[]>([{ symbol: 'ETH-USDT', turnover: 0 }]); const [symbol, setSymbol] = useState('ETH-USDT'); const [selectedRange, setSelectedRange] = useState<(typeof chartRanges)[number]>('24h'); const [venue, setVenue] = useState('all'); const [filterKind, setFilterKind] = useState<LiquidationFilterKind>('notional'); const [filterMinimum, setFilterMinimum] = useState('10000'); const [data, setData] = useState<LiquidationChart | null>(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(true)
  ...
  useEffect(() => {
    const nextViewportKey = `${symbol}:${selectedRange}:${venue}`
    ...
    const query = new URLSearchParams({ symbol, range: selectedRange, exchanges: venue, filter: filterKind, min: String(minimum) })
  ...
  }, [symbol, selectedRange, venue, filterKind, filterMinimum, minimum, validMinimum])
  ...
  return <div className="liquidation-page"><div className="liquidation-top"><div><div className="eyebrow">PUBLIC LIQUIDATION FLOW · BINANCE / OKX</div><h2>爆仓气泡图</h2><p>OKX USDT 永续 K 线叠加公开爆仓事件。数据由交易所直连与备源持续补漏。</p></div><div className="liquidation-status"><span className={status?.exchanges.binance?.connected ? 'online' : ''} title={directTitle('binance')}>Binance 直连 · {directTime('binance')}</span><span className={status?.exchanges.okx?.connected ? 'online' : ''} title={directTitle('okx')}>OKX 直连 · {directTime('okx')}</span><span className={status?.fallback?.lastSuccess ? 'online' : ''} title={status?.fallback?.error}>备源同步 · {fallbackTime}</span></div></div><Card className="liquidation-chart-card"><div className="liquidation-controls"><label>币对<select value={symbol} onChange={event => setSymbol(event.target.value)}>{symbols.map(item => <option key={item.symbol}>{item.symbol}</option>)}</select></label><div className="range-buttons">{chartRanges.map(item => <button key={item} className={selectedRange === item ? 'active' : ''} onClick={() => setSelectedRange(item)}>{item}</button>)}</div><div className="venue-buttons">{[['all', '全部'], ['binance', 'Binance'], ['okx', 'OKX']].map(([id, label]) => <button key={id} className={venue === id ? 'active' : ''} onClick={() => setVenue(id)}>{label}</button>)}</div><div className="filter-controls"><span>过滤</span><div className="filter-buttons"><button className={filterKind === 'notional' ? 'active' : ''} onClick={() => changeFilterKind('notional')}>金额</button><button className={filterKind === 'quantity' ? 'active' : ''} onClick={() => changeFilterKind('quantity')}>数量</button></div><label>≥ <input aria-label="爆仓过滤阈值" type="number" min="0" step="any" value={filterMinimum} onChange={event => setFilterMinimum(event.target.value)} /> <b>{filterUnit}</b></label></div><span className="chart-gesture-hint">滚轮缩放 · 左键拖动横移</span></div><div className="collection-note">爆仓数据采集起始：{started} · 当前展示 {data?.events.length ?? 0} 个原始事件 · 筛选：{filterLabel}</div><div className="liquidation-chart-wrap"><div className="liquidation-chart" ref={element} />{error ? <div className="liquidation-empty error">{error}<small>请填写有效阈值，或检查爆仓数据服务是否可用。</small></div> : loading ? <div className="liquidation-empty">正在加载 {symbol} 图表数据…</div> : data && data.candles.length === 0 ? <div className="liquidation-empty">K 线正在从 OKX 公开流累积。连接建立后会自动刷新。</div> : null}</div></Card></div>
}
