...
function RiskPage() {
  ...
  return <>
    <div className="risk-banner"><div><div className="eyebrow">ETH · RISK MONITOR</div><h1>杠杆风险升级</h1><p>两项以上信号共振触发 · 仅研究提示，不构成交易建议</p></div><div className="risk-level"><span>当前等级</span><b>HIGH</b><em>↑ 较昨日升高</em></div></div>
    <div className="metrics">{[['价格', '$1,904', '+2.4%', 'green'], ['下方清算密集区', '1823.5', '距现价 4.2%', 'amber'], ['未平仓合约', '$4.39B', '24h +11.8%', 'red'], ['Funding', '+0.0002%', '8h', 'violet']].map(([a,b,c,t]) => <Card key={a}><label>{a}</label><h2>{b}</h2><Pill tone={t}>{c}</Pill></Card>)}</div>
    <div className="two-columns"><Card><div className="card-heading"><div><div className="eyebrow">EXPLAINABLE SIGNALS</div><h2>共振触发依据</h2></div><Pill tone="red">2 / 4 维度</Pill></div>{[['清算结构迁移', '下方清算密集区向现价上移', '触发'], ['OI / Funding 异常', 'OI 持续抬升，Funding 维持正值', '触发'], ['价格接近清算区', '尚未进入近端阈值', '观察'], ['现货 / 衍生品背离', '现货未跟涨，需继续跟踪', '观察']].map(([a,b,c]) => <div className="signal" key={a}><span className={c === '触发' ? 'signal-on' : ''} /> <div><b>{a}</b><p>{b}</p></div><Pill tone={c === '触发' ? 'red' : 'muted'}>{c}</Pill></div>)}</Card><Card><div className="eyebrow">LATEST EVENT</div><h2>风险提示 #ETH-0812-04</h2><p className="detail-meta">触发于 14:32 · 数据快照 14:30</p><div className="detail-section"><label>影响说明</label><p>杠杆堆积与下方清算区距离缩短，若现货承接不足，短时波动可能放大。</p></div><div className="detail-section"><label>处理状态</label><Pill tone="amber">待研究员确认</Pill></div><button className="primary wide">标记为已阅</button></Card></div>
    <Card className="liquidation-monitor"><div className="card-heading"><div><div className="eyebrow">LIQUIDATION LEVELS · 30D</div><h2>清算点位</h2><p>ETH 清算热区速报，展示多空累计清算量与关键价格区间。</p></div><a className="monitor-link" href={liquidationMonitorURL} target="_blank" rel="noreferrer">在新窗口打开 ↗</a></div><div className="monitor-frame"><iframe title="ETH 清算点位监控（30日）" src={liquidationMonitorURL} loading="lazy" /></div></Card>
  </>
}
...
type LiquidationExchangeStatus = { connected: boolean, lastEvent?: string, lastDirectEvent?: string, lastFallbackEvent?: string, lastMessage?: string, error?: string }
type LiquidationChart = { symbol: string, collectionStartedAt?: string, candles: LiquidationCandle[], events: LiquidationEvent[], status: { exchanges: Record<string, LiquidationExchangeStatus>, fallback?: { enabled: boolean, lastSuccess?: string, error?: string } } }
...
function LiquidationBubblePage() {
  const element = useRef<HTMLDivElement>(null); const instance = useRef<echarts.ECharts | null>(null); const zoomWindow = useRef<ZoomWindow | null>(null)
  const [symbols, setSymbols] = useState<LiquidationSymbol[]>([{ symbol: 'ETH-USDT', turnover: 0 }]); const [symbol, setSymbol] = useState('ETH-USDT'); const [selectedRange, setSelectedRange] = useState<(typeof chartRanges)[number]>('24h'); const [venue, setVenue] = useState('all'); const [data, setData] = useState<LiquidationChart | null>(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(true)
  ...
  useEffect(() => { const controller = new AbortController(); zoomWindow.current = null; setLoading(true); setError(''); const query = new URLSearchParams({ symbol, range: selectedRange, exchanges: venue }); void fetch(`/api/v1/liquidations/chart?${query}`, { signal: controller.signal }).then(async response => { const result = await response.json() as LiquidationChart & { error?: string }; if (!response.ok) throw new Error(result.error || '无法读取爆仓数据'); setData(result) }).catch(fetchError => { if ((fetchError as Error).name !== 'AbortError') { setData(null); setError((fetchError as Error).message) } }).finally(() => setLoading(false)); return () => controller.abort() }, [symbol, selectedRange, venue])
  ...
  useEffect(() => { const chart = instance.current; if (!chart) return; const candles = data?.candles ?? []; const events = data?.events ?? []; const zoom = zoomWindow.current; const zoomState = zoom ? { start: zoom.start, end: zoom.end } : {}; const bubbles = (side: 'long' | 'short', color: string, name: string) => ({ name, type: 'scatter' as const, data: events.filter(item => item.side === side).map(item => [item.occurredAt, item.price, item.notional, item.exchange, item.quantity]), symbolSize: (value: unknown[]) => Math.min(46, Math.max(8, Math.log10(Number(value[2]) + 1) * 5)), itemStyle: { color, opacity: .72 }, z: 6 }); chart.setOption({ animation: false, backgroundColor: 'transparent', grid: { left: 58, right: 28, top: 44, bottom: 68 }, legend: { top: 8, textStyle: { color: '#9eacc1', fontSize: 11 }, data: ['OKX 5m K线', '多头爆仓', '空头爆仓'] }, tooltip: { trigger: 'axis', axisPointer: { type: 'cross' }, backgroundColor: '#101925', borderColor: '#3a4b65', textStyle: { color: '#e9eef7' }, formatter: (raw: unknown) => { const items = raw as Array<{ seriesName: string, value: unknown[] }>; return items.filter(item => item.seriesName !== 'OKX 5m K线').map(item => `<b>${item.seriesName}</b><br/>时间：${new Date(String(item.value[0])).toLocaleString()}<br/>价格：${Number(item.value[1]).toLocaleString()}<br/>名义价值：${formatMoney(Number(item.value[2]))}<br/>交易所：${item.value[3]}<br/>数量：${Number(item.value[4]).toLocaleString()}`).join('<hr/>') } }, xAxis: { type: 'time', axisLine: { lineStyle: { color: '#415069' } }, axisLabel: { color: '#8492a7' }, splitLine: { show: false } }, yAxis: { scale: true, axisLine: { lineStyle: { color: '#415069' } }, axisLabel: { color: '#8492a7' }, splitLine: { lineStyle: { color: '#243044' } } }, dataZoom: [{ type: 'inside', xAxisIndex: 0, zoomOnMouseWheel: true, moveOnMouseMove: true, moveOnMouseWheel: false, preventDefaultMouseMove: true, ...zoomState }, { type: 'slider', xAxisIndex: 0, height: 18, bottom: 18, borderColor: '#314058', fillerColor: '#725cbb55', textStyle: { color: '#8492a7' }, ...zoomState }], series: [{ name: 'OKX 5m K线', type: 'candlestick', data: candles.map(item => [item.openTime, item.open, item.close, item.low, item.high]), itemStyle: { color: '#57c995', color0: '#ec6e83', borderColor: '#57c995', borderColor0: '#ec6e83' } }, bubbles('long', '#f06e84', '多头爆仓'), bubbles('short', '#55d7a1', '空头爆仓')] }, true); if (candles.length === 0) chart.clear() }, [data])
  useEffect(() => { const protocol = window.location.protocol === 'https:' ? 'wss' : 'ws'; const socket = new WebSocket(`${protocol}://${window.location.host}/api/v1/liquidations/stream?${new URLSearchParams({ symbol, exchanges: venue })}`); socket.onmessage = message => { const result = JSON.parse(message.data) as { type?: string, event?: LiquidationEvent }; if (result.type === 'liquidation' && result.event) setData(current => current && current.symbol === result.event?.symbol ? { ...current, events: [...current.events, result.event] } : current) }; return () => socket.close() }, [symbol, venue])
  const started = data?.collectionStartedAt ? new Date(data.collectionStartedAt).toLocaleString() : '尚未收到爆仓事件'; const status = data?.status; const directTime = (exchange: 'binance' | 'okx') => status?.exchanges[exchange]?.lastDirectEvent ? new Date(status.exchanges[exchange].lastDirectEvent).toLocaleTimeString() : '未收到'; const fallbackTime = status?.fallback?.lastSuccess ? new Date(status.fallback.lastSuccess).toLocaleTimeString() : '同步中'
  return <div className="liquidation-page"><div className="liquidation-top"><div><div className="eyebrow">PUBLIC LIQUIDATION FLOW · BINANCE / OKX</div><h2>爆仓气泡图</h2><p>OKX USDT 永续 K 线叠加公开爆仓事件。数据由交易所直连与备源持续补漏。</p></div><div className="liquidation-status"><span className={status?.exchanges.binance?.connected ? 'online' : ''} title={status?.exchanges.binance?.error}>Binance 直连 · {directTime('binance')}</span><span className={status?.exchanges.okx?.connected ? 'online' : ''} title={status?.exchanges.okx?.error}>OKX 直连 · {directTime('okx')}</span><span className={status?.fallback?.lastSuccess ? 'online' : ''} title={status?.fallback?.error}>备源同步 · {fallbackTime}</span></div></div><Card className="liquidation-chart-card"><div className="liquidation-controls"><label>币对<select value={symbol} onChange={event => setSymbol(event.target.value)}>{symbols.map(item => <option key={item.symbol}>{item.symbol}</option>)}</select></label><div className="range-buttons">{chartRanges.map(item => <button key={item} className={selectedRange === item ? 'active' : ''} onClick={() => setSelectedRange(item)}>{item}</button>)}</div><div className="venue-buttons">{[['all', '全部'], ['binance', 'Binance'], ['okx', 'OKX']].map(([id, label]) => <button key={id} className={venue === id ? 'active' : ''} onClick={() => setVenue(id)}>{label}</button>)}</div><span className="chart-gesture-hint">滚轮缩放 · 左键拖动横移</span></div><div className="collection-note">爆仓数据采集起始：{started} · 当前展示 {data?.events.length ?? 0} 个原始事件</div><div className="liquidation-chart-wrap"><div className="liquidation-chart" ref={element} />{error ? <div className="liquidation-empty error">{error}<small>请配置 RAG_UI_DATABASE_URL，并等待两所公开数据流建立连接。</small></div> : loading ? <div className="liquidation-empty">正在加载 {symbol} 图表数据…</div> : data && data.candles.length === 0 ? <div className="liquidation-empty">K 线正在从 OKX 公开流累积。连接建立后会自动刷新。</div> : null}</div></Card></div>
}
